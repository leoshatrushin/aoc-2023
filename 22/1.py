...
for cz in range(1,x[2][1]+1):
    if not cz in z: continue
    for b in z[cz]:
        if cz==x[2][1]:t+=1;continue
        blist=[]
        dz=max(y[b][2])-min(y[b][2])
        for i in range(min(y[b][0]),max(y[b][0])+1):
            for j in range(min(y[b][1]),max(y[b][1])+1):
                if m[cz+1+dz][i][j]:blist.append(m[cz+1+dz][i][j])
        add=1
        for b2 in blist:
            stop=0
            for i in range(min(y[b2][0]),max(y[b2][0])+1):
                for j in range(min(y[b2][1]),max(y[b2][1])+1):
                    if m[cz+dz][i][j] and m[cz+dz][i][j]!=b:stop=1;break
                if stop:break
            if not stop:add=0;break
        if add:t+=1
print(t)
